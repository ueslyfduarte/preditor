...
class MaquinaOverallV28:
    ...
    def calcular_bloco_resistencia(self, time_pressao: Dict[str, float], liga_pressao: Dict[str, float], jogo_atual_time: Dict[str, Any]) -> Dict[str, float]:
        ...
        chutes_sofridos_time = jogo_atual_time['historico_defensivo_jogos'][0]['chutes_sofridos'] if jogo_atual_time.get('historico_defensivo_jogos') else 10.0
        chutes_gol_sofridos_time = jogo_atual_time['historico_defensivo_jogos'][0]['chutes_gol_sofridos'] if jogo_atual_time.get('historico_defensivo_jogos') else 4.0
...
# Estilização para remover a tela branca e deixar o visual profissional escuro
st.markdown("""
    <style>
...
# =====================================================================
# DIVISÓRIA 2: MOTOR ESTATÍSTICO COMPLETO (CLASSES E LÓGICA)
# =====================================================================
...
class PainelInicialV28:
    ...
    def processar_time(self, dados: Dict[str, Any], adversario_nivel_atual: int, mando: str) -> Dict[str, Any]:
        nivel_atual = self.definir_nivel_posicao(dados['posicao_real'])
        nivel_dinamico = self.calcular_nivel_dinamico(dados['posicao_pre_campeonato'], dados['posicao_real'])
        
        pontos_f5 = sum([self.calcular_pontos_retro(r, 'geral', nivel_atual, adversario_nivel_atual) for r in dados['historico_5j_caracteres']])
        pontos_f10 = sum([self.calcular_pontos_retro(r, 'geral', nivel_atual, adversario_nivel_atual) for r in dados['historico_geral_10']])
        pontos_mando = sum([self.calcular_pontos_retro(r, mando, nivel_atual, adversario_nivel_atual) for r in dados['historico_mando_5']])
        
        return {
            "posicao_real": dados['posicao_real'],
            "nivel_real": nivel_atual,
            "nivel_dinamico_fmp": nivel_dinamico,
            "aproveitamento_f5_retro": pontos_f5 / 15.0,
            "aproveitamento_f10_retro": pontos_f10 / 30.0,
            "aproveitamento_mando_retro": pontos_mando / 15.0
        }
...
class IndiceMomentoV28:
        ...
        for r in historico:
            if r == 'V': 
                ...
                if mando == 'visitante': 
                    ...
                    pontos_acumulados += 2.0 if nivel_time <= nivel_adversario else 0.0 if (nivel_time==1 and nivel_adversario==4) else 1.0
    ...
    def calcular_im_final(self, dados_time: Dict[str, Any], adversario_nivel: int, mandante: bool) -> float:
        def obter_nivel(p): 
            return 1 if p<=5 else 2 if p<=10 else 3 if p<=15 else 4
        nv_time = obter_nivel(dados_time['posicao_real'])
        mando_str = 'mandante' if mandante else 'visitante'
...
# =====================================================================
# DIVISÓRIA 4: MASSA DE DADOS DE ENTRADA SIMULADA (PADRÃO CONTRATO V2.8)
# =====================================================================
...
dados_partida_completa = {
    ...
    "mandante": {
        "nome": "Time Mandante A", 
        "liga_origem": "Liga Nacional 1",
        "posicao_real": 3, 
        "posicao_momentanea": 1, 
        "posicao_pre_campeonato": 2,
        ...
        "historico_mando_3": ['V', 'E', 'V'], 
        "historico_mando_5": ['V', 'E', 'V', 'D', 'V'], 
        "historico_5j_caracteres": ['V', 'V', 'E', 'D', 'V'],
        "historico_geral_3": ['V', 'V', 'E'], 
        "historico_geral_5": ['V', 'V', 'E', 'D', 'V'], 
        "historico_geral_10": ['V', 'V', 'E', 'D', 'V', 'E', 'V', 'D', 'V', 'V'],
    ...
    "visitante": {
        "nome": "Time Visitante B", 
        "liga_origem": "Liga Nacional 2",
        "posicao_real": 16, 
        "posicao_momentanea": 18, 
        "posicao_pre_campeonato": 10,
        "historico_ofensivo_jogos": [
        ...
        "historico_mando_3": ['D', 'E', 'D'], 
        "historico_mando_5": ['D', 'E', 'D', 'V', 'D'], 
        "historico_5j_caracteres": ['D', 'D', 'E', 'D', 'V'],
        "historico_geral_3": ['D', 'D', 'E'], 
        "historico_geral_5": ['D', 'D', 'E', 'D', 'V'], 
        "historico_geral_10": ['D', 'D', 'E', 'D', 'V', 'E', 'D', 'D', 'V', 'D'],
...
# =====================================================================
# DIVISÓRIA 5: EXECUÇÃO DOS MÓDULOS E APRESENTAÇÃO DOS RESULTADOS
# =====================================================================
...
st.write(f"• **Análise Ofensiva:** O poder de fogo do mandante ({res_atq_m['Nota_Ataque']} pts) enfrenta uma linha defensiva vulnerável do visitante ({res_def_v['Nota_Defesa']} pts), criando um cenário técnico propício para gols do mandante.")
...
# 1. Mercado Vitória Mandante (1X2 Casa)
...
# 2. Mercado Vitória Visitante (1X2 Fora)
...
# 6. Mercado Gols FT - Over 2.5
if odds["over_25_ft_pct"] is not None:
    pct_metodo_o25ft = min(95.0, max(5.0, forca_gols_geral - 15.0))
    dif = pct_metodo_o25ft - odds["over_25_ft_pct"]
    linhas_mercados.append(["Gols: Over 2.5 FT", f"{odds['over_25_ft_pct']}%", f"{pct_metodo_o25ft:.1f}%", f"{dif:+.1f}%", "🟢 VALOR OVER 2.5" if dif>=5.0 else "🔴 EVITAR / IR NO UNDER" if dif<=-5.0 else "🟡 Neutro"])

# 7. Mercado Ambas Marcam (Sim)
if odds["ambas_marcam_sim_pct"] is not None:
    pct_ambas_metodo = min(95.0, max(5.0, (res_atq_m["Nota_Ataque"] + res_atq_v["Nota_Ataque"]) / 2 - 12.0))
    dif = pct_ambas_metodo - odds["ambas_marcam_sim_pct"]
    linhas_mercados.append(["Ambas Marcam: SIM", f"{odds['ambas_marcam_sim_pct']}%", f"{pct_ambas_metodo:.1f}%", f"{dif:+.1f}%", "🟢 VALOR AMBAS" if dif>=5.0 else "🔴 EVITAR" if dif<=-5.0 else "🟡 Neutro"])

# 8. Mercado de Linha de Escanteios Dinâmico
...
# Proteção de Empate Anula a Aposta (DNB) avaliada pelo app
